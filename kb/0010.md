# Repository Knowledge Base (chunk 0010)
...
type UpsertResp = { ok: true; id: string } | { error: string };
type CuspsResp =
  | { ok: true; count: number; system: HouseSystem; approx: string | null }
  | { error: string };

export default function PeoplePanel({ houseSystem, defaultPerson }: PeoplePanelProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // 1) upsert persona
      const upRes = await fetch('/api/people/upsert', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          id: form.id,
          ...
          birth_tz_offset_minutes:
            form.birth_tz_offset_minutes != null ? Number(form.birth_tz_offset_minutes) : null,
          birth_lat: form.birth_lat != null ? Number(form.birth_lat) : null,
          birth_lon: form.birth_lon != null ? Number(form.birth_lon) : null,
        } satisfies PersonFormState),
      });

      const upJson = (await upRes.json()) as UpsertResp;
      if (!upRes.ok || 'error' in upJson) {
        throw new Error(('error' in upJson ? upJson.error : `HTTP ${upRes.status}`));
      }
      const personId = (upJson as { ok: true; id: string }).id;
      setForm((s) => ({ ...s, id: personId }));

      // 2) calcolo & salvataggio cuspidi
      ...
      const msg = (err as Error)?.message ?? 'Errore inatteso';
      setError(msg);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="flex flex-col gap-1">
          <span className="text-sm text-gray-600">Fuso (offset minuti)</span>
          <input
            type="number"
            step="1"
            value={form.birth_tz_offset_minutes ?? 0}
            onChange={(e) => set('birth_tz_offset_minutes', Number(e.target.value))}
            className="rounded border px-2 py-1"
            placeholder="es. 60"
          />
        </label>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-sm text-gray-600">Ora (HH:MM)</span>
          <input
            type="time"
